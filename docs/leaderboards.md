...
- **Header.** One line: at the left the eyebrow of what follows ("Rotation · {n} casts", counting button presses only, or "Healing"), at the right the score in Gowun 30px. The build row above already carries the score, so here it is a figure beside the rotation, not a banner over it. Rows flagged `modifier: true` turn the score into an equation in payload order, since the backend applies modifiers in sequence against the running score: Energy Regen is a `×` term labelled from `modifierInfo` (never parsed from the name), set, echo and bonus modifiers are additive terms, and a factor that follows an additive term wraps what precedes it in parentheses.
...
- **Abilities** sort by damage: tab disc with the type arc, name with a secondary line, casts, bar on a faint track, share, damage. The bar lane flexes and the name column caps near 22rem, so the cast count sits beside the name and extra width goes to the bars. The secondary line is "{Tab} → {Type}" only when a scored type is not native to the tab (Normal Attack natively deals Basic and Heavy Attack, a Resonance Skill deals Resonance Skill, and so on), otherwise the type alone with its legend label; status rows read "Negative Status", and their "No crit or DMG bonus", "No crit" or "No DMG bonus" tag (from `noCrit` and `bypassDmgBonus`) says what that exempts them from. The whole row is the `aria-expanded` toggle. Open, a row lifts out as a card and its bar splits by hit instead of by type; the hit rows beneath carry name ×count, per-event MV, share and damage and no bar of their own, and a hit row and its piece light together. A row without children shows "{MV} MV"; "Scales with {stat}" appears only when it differs from the stat carrying most of the damage; the last line is "Casts {ranges} of {n}" beside a full-width miniature of the ribbon with this row's casts lit ("Position in the rotation" for status rows, which have no cast numbers).
