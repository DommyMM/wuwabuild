...
- **Home (`/`)** — landing surface, rebuilt 2026-06 on the Enka/Akasha model (tool-first, art-backed, personality contained in the updates log). Hero: a rotating showcase (6.5s crossfade, pause on hover, reduced-motion aware) of each character's record run, splash art dimmed into the dark with an element-tinted glow plus a "Highest damage" record panel (character, track, weapon, damage, owner, reign) linking to the board; the tagline (canonical SEO h1, see `seo-audit-findings.md` Phase 3), an Enka-style UID/username search pill (hits `GET /profile?q=` on the `api.wuwa.build` gateway), and quick links. Body: "Top boards." (8 most contested, deduped to one board per character) beside "# updates" (latest changelog entries verbatim, header links to `/changelog`, same kind chips). Then "How to use." + the FAQ printed flat. No home-specific footer; the global `Footer.tsx` carries the colophon info. Community contact is always the Discord server invite, never DM handles. Headlines are sentence-case-with-period. No em dashes anywhere in site copy (owner preference).
