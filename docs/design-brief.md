...
- **Home (`/`)** — landing surface, rebuilt 2026-06 on the Enka/Akasha model (tool-first, art-backed, personality contained in the updates log). Hero: a rotating showcase (6.5s crossfade, reduced-motion aware) of each character's record run, splash art dimmed into the dark with an element-tinted glow plus a "Board record" panel (character + track as one `char-sig` title matching `LeaderboardCharacterHeader`, sequence pill trailing, weapon, damage, owner, reign); the tagline (canonical SEO h1, see `seo-audit-findings.md` Phase 3), an Enka-style UID/username search pill (hits `GET /profile?q=` on the `api.wuwa.build` gateway), and quick links. Body: "Top leaderboards" (9 boards, ordered by entry count, deduped to one board per character) beside "# updates" (latest changelog entries verbatim, header links to `/changelog`, same kind chips). Then "How to use" + the FAQ printed flat. No home-specific footer; the global `Footer.tsx` carries the colophon info. Community contact is always the Discord server invite, never DM handles. Headlines and other short UI copy omit ending punctuation; multi-sentence paragraphs keep it. No em dashes anywhere in site copy (owner preference).
  ...
  - The board list is ordered by entry count, and entry count is its ONLY figure. Record damage and its holder were tried here twice and removed twice: cross-board damage is not comparable, it duplicated the hero card, and any unlabeled number in the sorted column reads as the ranking. The record story lives on the hero card and the board page.
  - "How to use" shows a real wuwa-bot card (`public/images/wuwa-bot-card.webp`, the owner's Hiyuki card) beside three terse steps. The image is a noninteractive figure so it cannot be mistaken for a zoom control; the explicit Import link in step 2 owns navigation. Card download lives in the FAQ.
  - The first hero slide's move profile is server-fetched (`prefetchBuildMoves` in `lbServer.ts`) and baked into the ISR HTML via `initialProfile`, so the signature graphic exists at first paint. Later slides fetch lazily.
  - The search placeholder cites the first slide's record holder (name AND uid) so it teaches both accepted input forms — always a real, currently searchable player, never an invented one.
