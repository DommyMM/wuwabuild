...
Two chips are **content-sized** (`shrink`, basis auto) so each takes only what its name needs and gives the rest back: "Midnight Veil" is 161px, not half the row. Nothing is `shrink-0` and there are no `max-w` caps. A single rigid chip would dump the entire overflow onto its neighbour, and an even split pads short names out with dead space; both read as accidents rather than decisions.

Size and wrap mode are one value for the whole row, resolved in a two-pass `useLayoutEffect`: the largest `SET_NAME_ONE_LINE_SIZES_PX` that holds every name on one line, and failing that the largest `SET_NAME_WRAPPED_SIZES_PX` that keeps every name inside `SET_NAME_MAX_LINES`. **One line is always preferred over two** — dropping a size is far less visible than a wrap. It mirrors the weapon-name fit in `WeaponGroup`: written straight onto the nodes as a layout fact, re-run on `document.fonts.ready` because first paint can be in the fallback face, and `overflow-hidden` on the span is what makes the scroll metrics report real overflow. `whitespace-nowrap` is the class default so the common row is already correct at first paint. Do not decide per chip; two different text sizes side by side is the failure mode this replaced.

Measured 2026-09-11 (Plus Jakarta wght 600, design pixels) against all 6,696 live two-set builds: **42% fit one line at the full 14px, and stepping only as far as 12px reaches 74%**. The ladder earns its keep on the near-misses — "Dream of the Lost" + "Havoc Eclipse", the second most common pair in production, overruns by 0.02px at 14px and clears comfortably at 13px. Every pair that can exist balances onto two lines at 14px, so the wrapped pass normally lands on its first rung too.
...
`COMPACT_SET_NAMES` applies **only** when three sets are active; one- and two-set rows always render the full name. Two consequences when editing it: a three-piece set can never reach the map (`Thread of Severed Fate` was removed 2026-09-12 as dead), and no set can show a `5` badge there. It is keyed on the translated name, so it only matches in English; other locales fall through to the full name and rely on the ladder. The 9px floor is sized for this row: "Tidebreaking" and "Rejuvenating" are unbreakable single words that still overflow 66px at 10px, and they fit at 9px. `Song of Feathered Trace` is the one English name that needs a compact entry rather than a smaller size.

The old `WRAPPING_SET_NAME_LENGTH` / `CROWDED_SET_NAME_LENGTH` character-count thresholds were removed 2026-09-11. They inverted the size ordering: names at or above 18 characters dropped to `text-xs` **and** a 168px cap, so the longest names produced the narrowest chips and mid-length names like "Rejuvenating Glow" (17) produced the widest. 19% of possible pairs overflowed the column, including the second most common pair in production (Dream of the Lost + Havoc Eclipse, which missed by 0.02px).
...
Bundled splash files ship at a 1080p / WebP q80 budget, not their 4K source resolution: the art panel renders at 600 design-units tall (max zoom 1.4) and exports at dpr 1, so nothing ever reads more than ~1500x840 source pixels, and 1080p keeps ~2x headroom. After dropping new splash art into `public/images/splash/`, run `node scripts/compress_splash.mjs` (idempotent; skips files already within budget) and add the stem to `BUNDLED_SPLASH_STEMS`.

Profile expansion is deliberately staged. The page widens over 150ms while the splash download is already warming: `handleToggleExpand` calls `warmBundledSplashArt` at row click, so the art fetch runs in parallel with the build-detail request instead of after it. Once layout is stable and build details are available, `ProfileCard` mounts at its final width but remains invisible until `useAdaptiveCardColors` has sampled the initial art. The prepared card then reveals downward using only `clip-path`, opacity, and transform over 220ms. The loading indicator (text-free accent dots) follows standard progress-indicator timing: it mounts only if loading is still active 200ms after the width stage settles (the width motion is choreography, never counted as waiting), and once shown stays a minimum 400ms (the reveal waits out that minimum) so it never flickers. Warm reopens and cached opens resolve inside the delay and show no indicator at all. Do not reintroduce a simultaneous `height: 0` to `height: auto` animation around the 1440px card: it makes `CardScaler`, the table scrollport observer, and the expansion animation chase one another's layout measurements.
...
| Group | Contents | Spec |
|---|---|---|
| Grade | `top` kicker, percentile, `#rank / total` | Percentile `font-gowun 700 25px` in tier color with glow, the only quality signal on the card. Total abbreviates at five digits (`formatTotal`: "1.6k", "95.7k") so the line survives boards growing 100x. Percentile stays the hero at every rank (a podium "#1 hero" variant was tried 2026-06-10 and reverted by owner preference). A `TOP wuwa.build` kicker was tried 2026-09-05 and removed by owner preference (obtrusive). The grade and the board identity are one `Link` to the build's row on that board (`boardHref`, computed in `ProfileCard` from the active board); hover turns the rank and track label accent and underlines the label, and the browser shows the board URL. Supports stay outside the link, they carry their own hover cards. |
...
**Long weapon names shrink instead of truncating.** `WeaponGroup` measures the name span in a layout effect (kept inline, it has one caller): it measures the real DOM at 24px and steps the inline `font-size` down (floor 16px) until the text fits the slot beside the icon, re-measuring once web fonts are ready. Measured 2026-09-06 in Plus Jakarta: the slot is ~250px and 9 of 122 English names (92 of 610 Latin-script) overflow it at 24px, so this is not an edge case. `truncate` stays on the span as the floor's safety net and keeps the no-wrap invariant above; the export sees the same inline size the screen does.

The site mark ("wuwa.build") is bare shadowed text at `text-sm` pinned bottom-left of the splash panel (`CharacterPanel`), stacked under the optional art-credit input in the editor. Shared cards are the distribution loop; keep the mark legible at embed size.
...
- The stage is placement-agnostic. `ProfileBuildExpanded` is the table-row shell around it; `ProfileFeaturedBuild` is the region between the rankings shelf and the filters for builds that arrive without a row (tile click, `?buildId=` deep link, echo inventory cross-link). `initialStandingKey` (`weaponId:trackKey`, from the tile or `?board=`) opens the card on the board the reader was just looking at instead of `pickDefaultBoard`'s choice; an unknown key falls through to the default.
- Echo chip highlighting is one helper everywhere: `getAvailablePreferredSubstats(echoPanels, character.preferredStats)` in `lib/calculations/rollValues.ts`. `ProfileCard` and the leaderboard expansion let the reader retune it through their summary pills; the editor card applies it fixed (no pill row), so the four surfaces agree on which chips are gold for a given build. `preferredStats` itself is derived per character by `scripts/sync_characters.py` (see `scripts/CDN_SYNC.md`).
