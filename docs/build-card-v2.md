...
`ActiveSetsSection` sits at the bottom of the 480px (`w-120`) info column. After `pl-4` the row has 464px, the CV chip takes 83px, and `gap-2` takes the rest, leaving **364.9px of chip budget**. From two sets up, chips are `flex-1` so they split that budget evenly (~182px each, ~113px of text after icon, gaps and piece badge). Nothing is `shrink-0` and there are no `max-w` caps: a single rigid chip would dump the entire overflow onto its neighbour, which is what made wrapping look like a glitch rather than a decision.

Size is one value for the whole row, picked by the `SET_NAME_SIZES_PX` ladder in a `useLayoutEffect` that measures `scrollHeight`/`scrollWidth` against `SET_NAME_MAX_LINES`. It mirrors the weapon-name fit in `WeaponGroup`: written straight onto the nodes as a layout fact, re-run on `document.fonts.ready` because first paint can be in the fallback face, and `overflow-hidden` on the span is what makes the scroll metrics report real overflow. Do not size chips independently; two different text sizes side by side is the failure mode this replaced.

Measured 2026-09-11 (Plus Jakarta wght 600, design pixels): **32 of the 34 English set names balance onto two lines at the full 14px**, so the ladder is a fallback rather than the common path. Only "Song of Feathered Trace" (13px) and "Shadow of Shattered Dreams" (12px) step down in a two-set row.

Three active sets is always Adam + 2pc + 2pc, verified against all 37,954 stored builds: every 3-set row contains Shadow of Shattered Dreams, and no build has four. That row switches to tighter chrome and `COMPACT_SET_NAMES`, which trades the "X of Y" prefix away before it trades size, because three chips share only ~66px of text each. The 9px floor exists for it: "Tidebreaking" and "Rejuvenating" are unbreakable single words that still overflow 66px at 10px. `Song of Feathered Trace` is the one English name that needs a compact entry rather than a smaller size.
