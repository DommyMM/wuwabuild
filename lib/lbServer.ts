...
// Fetches directly from LB_URL (bypasses the /api/lb proxy) with short TTL caching.

import { isRecord, toFiniteNumber, parseBuildRowEntry, parseLeaderboardEntry, LBBuildRowEntry, LBListBuildsResponse, LBCharacterOverview, LBWeaponTop, LBLeaderboardEntry, LBLeaderboardResponse } from './lb';
    ...
    for (const raw of rawChars) {
      ...
      result.push({
        ...
        weaponIds: Array.isArray(raw.weaponIds)
          ...
          : [],
        sequences: Array.isArray(raw.sequences)
          ? raw.sequences.filter((v): v is string => typeof v === 'string')
          : [],
  ...
  try {
    ...
    return {
      ...
      sequences: Array.isArray(payload.sequences)
        ? (payload.sequences as unknown[]).filter((v): v is string => typeof v === 'string')
        : [],
    };
  } catch (err) {
