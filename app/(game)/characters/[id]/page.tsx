        ...
        <main className="bg-background">
                                ...
                                <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                                    <div>
                                        ...
                                        <div className="mt-4 flex flex-wrap items-center gap-2">
                                            {char.title && (
                                                <span className="rounded border border-border bg-black/24 px-2.5 py-1 text-xs text-text-primary/60">
                                                    {char.title}
                                                </span>
                                            )}
                                            {rawChar.tags?.slice(0, 4).map((tag) => (
                                                <span
                                                    key={tag.id}
                                                    className="inline-flex items-center gap-1.5 rounded border border-border bg-black/24 px-2.5 py-1 text-xs text-text-primary/60"
                                                >
                                        ...
                                        <div className="mt-6 flex flex-wrap gap-2">
                                            {hasLeaderboard && (
                                                <Link className="gold-glow rounded border border-accent/45 bg-accent px-4 py-2 text-sm font-semibold text-background transition-colors hover:bg-accent-hover" href={`/leaderboards/${id}`}>
                                                    View leaderboard
                                                </Link>
                                            )}
                                            <Link className="gold-glow rounded border border-border bg-white/3 px-4 py-2 text-sm font-semibold text-text-primary/82 transition-colors hover:border-accent/45 hover:text-accent" href="/builds">
                                                Browse builds
                                            </Link>
                                        </div>
                                    </div>
            ...
            {insightProse && (
                <section className="mx-auto max-w-360 px-3 pt-4 md:px-16">
                    <div className="rounded-xl border border-border bg-background-secondary/55 p-5 md:p-6">
                        <p className="text-2xs font-semibold uppercase tracking-[0.24em] text-accent">Leaderboard insight</p>
