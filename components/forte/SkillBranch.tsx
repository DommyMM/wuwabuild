    ...
    <div className={`flex flex-col items-center ${className}`}>
      ...
      {hasNodes ? (
        <>
          ...
          {/* Connection line */}
          <div className={`${connectorClass} w-[2px] bg-text-primary`} />
            ...
            return (
              <React.Fragment key={pos}>
                <button
                  ...
                  className={`group relative flex items-center justify-center cursor-pointer -my-0.5 ${inherentFrameClass} ${frameBg} bg-contain bg-center bg-no-repeat transition-all duration-200 hover:scale-105`}
                ...
                >
                  ...
                  <img
                    src={inherentIcon}
                    alt={`Inherent ${i + 1}`}
                    className={`${compact ? 'h-6 w-6' : 'h-8 w-8'} object-contain invert transition-all
                      ${active ? 'opacity-100 drop-shadow-[0_0_4px_rgba(166,150,98,0.6)]' : 'opacity-40'}`}
                    draggable={false}
                  />
                </button>
                <div className={`${inheritedConnectorClass} w-[2px] bg-text-primary`} />
              </React.Fragment>
            );
          })}
        </>
      ...
      >
        <img
          src={skillIcon}
          alt={skillName}
          className={`relative ${compact ? 'h-7 w-7' : 'h-10 w-10'} object-contain invert`}
          draggable={false}
          onError={(e) => {
      ...
      <div className={`relative z-10 flex flex-col items-center ${compact ? 'gap-1.5' : 'gap-2'}`}>
        <div className={`flex items-center gap-1 ${compact ? 'text-xs' : 'text-sm'} text-text-primary/70`}>
          ...
          {isEditing ? (
            ...
            <button
              ...
              className={`${compact ? 'min-w-6 text-xs' : 'min-w-7 text-sm'} rounded border border-border bg-background px-1 py-0.5 text-center font-semibold text-accent transition-colors hover:border-accent`}
            >
              {level}
            </button>
          )}
        ...
        <input
          ...
          className={`forte-level-slider h-1.5 ${sliderWidthClass} cursor-pointer appearance-none rounded-full`}
          style={{
